...
export class AppConfigBuilder<TSchema extends z.ZodType<object>> {
  ...
  constructor(schema: TSchema, { configFolder = './config' }: { configFolder?: string } = {}) {
    this.#schema = schema;
  ...
  public addSource(source: string | EnvVarConfig): this {
    if (source instanceof EnvVarConfig) {
      ...
      const filePath = path.resolve(this.#appConfigFolderAbsolutePath, fileName);
  ...
  #addJsonSource(source: string): void {
    ...
    this.#partialConfig = AnyObject.parse(merge({}, this.#partialConfig, parsedJson));
  }

  #addYamlSource(source: string): void {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
    const parsedYaml = yaml.parse(source);
    this.#partialConfig = AnyObject.parse(merge({}, this.#partialConfig, parsedYaml));
  }
...
class EnvVarConfig {
  ...
  constructor({ prefix, prefixSeparator, separator, runtimeEnv }: EnvVarSourceOptions) {
