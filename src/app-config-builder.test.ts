...
describe('createAppConfig', () => {
  ...
  describe('.addSource', () => {
    describe('from environment variable', () => {
      it('loads source from process.env', () => {
        const appUrlMockEnv = injectEnvVar('APP_appURL', 'http://localhost:3000');
        const apiPortMockEnv = injectEnvVar('APP_api__port', '3000');

        const result = config.addSource(AppConfigBuilder.createEnvVarSource()).build();
      ...
      it('loads source from a different runtime', () => {
        // @ts-expect-error - .env is there
        const runtimeEnv = import.meta.env as Record<string, string | boolean | number | undefined>;

        const appUrlMockEnv = injectEnvVar('APP_appURL', 'http://localhost:4422', runtimeEnv);
      ...
      it('should consider custom prefix prefixSeparator and prop separator', () => {
        const appUrlMockEnv = injectEnvVar('TEST-appURL', 'http://localhost:3000');
    ...
    describe('from yaml', () => {
      ...
      it('should throw an error if the file does not exist', () => {
        try {
          ...
          expect(e.message).toEqual(
            expect.stringMatching(/File "[^]+.not-exist.yaml" does not exist/g),
          );
        }
      });
      ...
      it('should throw an error if tries to load an invalid resource', () => {
        try {
          ...
          const e = error as Error;
          expect(e.message).toEqual(
            expect.stringMatching(/Invalid source. Please provide a valid one./g),
          );
        }
      });
    ...
    describe('from json', () => {
      ...
      it('should add layer json files', () => {
        expect(config.addSource('base.json').addSource('dev.json').build()).toEqual({
        ...
        expect(config.addSource('dev.json').addSource('base.json').build()).toEqual({
          appURL: 'http://localhost:3000',
          api: {
            port: 3000,
          },
  ...
  it('should throw an error if pass an unsupported file', () => {
    try {
      ...
      const e = error as Error;
      expect(e.message).toEqual(expect.stringMatching(/File type "base.config" not supported./g));
    }
  });
});
