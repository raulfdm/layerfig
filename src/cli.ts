...
async function main() {
  if ((await exists(PATHS.userSchema.absolutePath)) === false) {
    throw new Error(`Schema file not found at path: ${PATHS.userSchema.absolutePath}`);
  }
  ...
  await writeFile(PATHS.output.absolutePath, JSON.stringify(jsonSchema, null, 2));
}
...
if (cli.flags.watch) {
  ...
  chokidar.watch(cli.flags.schema).on('change', async () => {
    await main().then(() => console.log('Schema file changed. JSON schema updated.'));
  });
}
