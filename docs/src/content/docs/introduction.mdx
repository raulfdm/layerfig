---
title: ''
description: 'Type-safe layered config. Merge files & env vars, validate with any schema, and get a runtime-checked object with full TypeScript IntelliSense'
tableOfContents: false
head:
  - tag: title
    content: Layerfig
hero:
  tagline: Type-safe layered config. Merge files & env vars, validate with any schema, and get a runtime-checked object with full TypeScript IntelliSense
  image:
    alt: logo
...
- Have declarative, isolated and per-environment configuration files (`.json`, or other file extensions via [parsers](/setup/sources#other-file-extensions)).
- Validate your configuration with your prefered schema validation library (zod, valibot, yup, etc.).
